...
from ..serializers.task_serializers import (
    TaskExecutionSerializer, TaskExecutionDetailSerializer,
    TaskStatusSerializer, SystemHealthSerializer, QueueMetricsSerializer,
    TaskMetricsSerializer, SlowTaskSerializer, FailureAnalysisSerializer,
    WebhookLogSerializer, ReportGenerationRequestSerializer,
    DataExportRequestSerializer, TaskRetryRequestSerializer,
    BulkTaskActionSerializer, TaskTrendSerializer
)
...
class TaskExecutionViewSet(ReadOnlyModelViewSet):
    ...
    def retry(self, request, public_id=None):
        ...
        if task_execution.status != TaskExecution.Status.FAILURE:
            return Response(
                {'error': 'Task can only be retried if it has failed'},
...
class SystemHealthView(APIView):
    ...
    def queues(self, request):
        ...
        try:
            ...
            serializer = QueueMetricsSerializer(
                [{'name': name, **metrics} for name, metrics in queue_metrics.items()],
                many=True
            )
            return Response(serializer.data)
    ...
    def slow_tasks(self, request):
        ...
        try:
            ...
            return Response(
                {'error': f'Failed to get slow tasks: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def failures(self, request):
        ...
        try:
            ...
            return Response(
                {'error': f'Failed to get failure analysis: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class TaskMetricsView(APIView):
    ...
    def get(self, request, task_name=None):
        ...
        try:
            if task_name:
                ...
                serializer = TaskMetricsSerializer(
    ...
    def trends(self, request, task_name):
        ...
        try:
            ...
            return Response(
                {'error': f'Failed to get task trends: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class WebhookLogViewSet(ReadOnlyModelViewSet):
    ...
    def retry(self, request, public_id=None):
        ...
        if webhook_log.status == WebhookLog.Status.DELIVERED:
            return Response(
                {'error': 'Webhook has already been delivered'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {'error': f'Failed to retry webhook: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def failed(self, request):
        ...
        failed_webhooks = self.get_queryset().filter(
...
class ReportGenerationView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {'error': f'Failed to schedule report generation: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class DataExportView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {'error': f'Failed to schedule data export: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
