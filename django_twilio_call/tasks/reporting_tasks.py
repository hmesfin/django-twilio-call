...
def calculate_agent_metrics(self, agent_id: int, start_date_str: str = None, end_date_str: str = None):
    ...
    """
    from ..models import Agent, Call, AgentActivity
    ...
    break_activities = activities.filter(
        activity_type__in=[
            AgentActivity.ActivityType.BREAK_START,
            AgentActivity.ActivityType.BREAK_END
        ]
    ).order_by('created_at')
    ...
    metrics = {
        ...
        'activity_metrics': {
            ...
            'login_count': activities.filter(
                activity_type=AgentActivity.ActivityType.LOGIN
            ).count(),
            'status_changes': activities.filter(
                activity_type=AgentActivity.ActivityType.STATUS_CHANGE
            ).count(),
        },
...
def calculate_hourly_metrics():
    ...
    hourly_calls = Call.objects.filter(
