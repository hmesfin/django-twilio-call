...
def process_webhook_callback(self, webhook_data: Dict[str, Any], webhook_type: str):
    ...
    """
    try:
        from ..models import Call, CallRecording, WebhookLog
        ...
        if hasattr(self, 'update_state'):
            self.update_state(
                state='PROGRESS',
                meta={'current': 1, 'total': 4, 'status': f'Processing {webhook_type} webhook'}
            )
        ...
        if hasattr(self, 'update_state'):
            self.update_state(
                state='PROGRESS',
                meta={'current': 4, 'total': 4, 'status': 'Webhook processing completed'}
            )
...
def retry_failed_webhook(self, webhook_log_id: int, max_retries: int = 3):
    ...
    try:
        ...
        if hasattr(self, 'update_state'):
            self.update_state(
                ...
                meta={
                    'current': 1,
                    'total': 3,
                    'status': f'Retrying webhook (attempt {webhook_log.retry_count + 1})'
                }
            )
        ...
        if hasattr(self, 'update_state'):
            self.update_state(
                state='PROGRESS',
                meta={'current': 3, 'total': 3, 'status': 'Retry queued'}
            )
...
def check_failed_webhooks():
    ...
    try:
        ...
        failed_webhooks = WebhookLog.objects.filter(
            ...
            Q(next_retry_at__isnull=True) | Q(next_retry_at__lte=cutoff_time)
        )
        ...
        return {
            'success': True,
            'failed_webhooks_found': failed_webhooks.count(),
...
def send_webhook_notification(self, webhook_data: Dict[str, Any], notification_url: str):
    ...
    try:
        if hasattr(self, 'update_state'):
            self.update_state(
                state='PROGRESS',
                meta={'current': 1, 'total': 3, 'status': 'Preparing webhook notification'}
            )

        # Prepare headers
        headers = {
            'Content-Type': 'application/json',
            'User-Agent': 'django-twilio-call-webhook/1.0'
        }
        ...
        if hasattr(self, 'update_state'):
            self.update_state(
                state='PROGRESS',
                meta={'current': 2, 'total': 3, 'status': 'Sending webhook notification'}
            )
        ...
        if hasattr(self, 'update_state'):
            self.update_state(
                state='PROGRESS',
                meta={'current': 3, 'total': 3, 'status': 'Webhook notification sent'}
            )
